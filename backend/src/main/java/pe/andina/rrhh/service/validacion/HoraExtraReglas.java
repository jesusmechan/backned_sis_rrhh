...
import pe.andina.rrhh.service.ParametroSistemaService;
...
public class HoraExtraReglas {
    ...
    public HoraExtraReglas(SolicitudHoraExtraRepository horaExtraRepository, ParametroSistemaService parametros) {
    ...
    private void validarTope(Integer idEmpleado, LocalDate fecha, BigDecimal cantidad) {
        ...
        if (dia.add(cantidad).compareTo(maxDia) > 0) {
            throw ApiException.badRequest("La cantidad de horas extras del día supera el máximo permitido (" + maxDia + ").");
        }
        if (semana.add(cantidad).compareTo(maxSemana) > 0) {
            throw ApiException.badRequest("La cantidad de horas extras de la semana supera el máximo permitido (" + maxSemana + ").");
        }
    }
}
