...
import pe.andina.rrhh.domain.AsientoLinea;
...
import pe.andina.rrhh.dto.AppDtos.AsientoLineaResponse;
import pe.andina.rrhh.dto.AppDtos.AsientoResponse;
import pe.andina.rrhh.dto.AppDtos.PlanillaDetalleResponse;
import pe.andina.rrhh.dto.AppDtos.PlanillaRequest;
import pe.andina.rrhh.dto.AppDtos.PlanillaResponse;
import pe.andina.rrhh.repo.AsientoContableRepository;
...
import pe.andina.rrhh.repo.PlanillaDetalleRepository;
import pe.andina.rrhh.repo.PlanillaRepository;
...
public class PlanillaService {
    ...
    public PlanillaResponse crear(PlanillaRequest request) {
        planillaRepository.findByAnioAndMes(request.anio(), request.mes()).ifPresent(p -> {
            throw ApiException.conflict("Ya existe una planilla para " + periodo(p.getAnio(), p.getMes()));
        });
        ...
        auditoriaService.registrar(SecurityUtils.current().getUsuario(), "CREAR", "PLANILLA", p.getIdPlanilla(), periodo(p.getAnio(), p.getMes()));
    ...
    public PlanillaResponse calcular(Integer id) {
        ...
        if (p.getEstado() == EstadoPlanilla.CERRADA) {
            throw ApiException.badRequest("La planilla cerrada no se vuelve a calcular");
        }
        if (p.getEstado() == EstadoPlanilla.ANULADA) {
            throw ApiException.badRequest("La planilla anulada no se puede calcular");
        }
        ...
        auditoriaService.registrar(SecurityUtils.current().getUsuario(), "CALCULAR", "PLANILLA", id, periodo(p.getAnio(), p.getMes()));
        return toResponse(p, detalleRepository.findByPlanilla_IdPlanillaOrderByIdDetalleAsc(id));
    }
    ...
    public PlanillaResponse cerrar(Integer id) {
        ...
        if (p.getEstado() != EstadoPlanilla.CALCULADA) {
            throw ApiException.badRequest("Solo se cierra una planilla calculada");
        }
        if (asientoRepository.findByPlanilla_IdPlanilla(id).isPresent()) {
            throw ApiException.conflict("Esta planilla ya tiene asiento contable");
        }
    ...
    public AsientoResponse obtenerAsiento(Integer id) {
        return toAsiento(asientoRepository.findConLineas(id).orElseThrow(() -> ApiException.notFound("Asiento no encontrado")));
    }
    ...
    private Planilla buscar(Integer id) {
        return planillaRepository.findById(id).orElseThrow(() -> ApiException.notFound("Planilla no encontrada"));
    }
